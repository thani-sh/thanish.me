...
const startWorld = (keymaps, renderer, ticker, ruleset, options) => {
  ...
  const createNewShape = (i) => {
    ...
    shape.x = Math.floor(
      (options.worldWidth / playersCount / 2) * (i * 2 + 1) - 1
    );
...
document.body.innerHTML = `
    <div style="position: absolute; top: 0; left: 0; width: 100%; background: #f6f8fa">
      <canvas id="game-canvas" style="display: block; margin: 0 auto; background: #fff">
      </canvas>
    </div>
  `;
